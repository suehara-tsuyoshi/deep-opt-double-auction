...
X_tst = data
fix_val = 0.5
Y_tst = np.full(X_tst.shape[0], fix_val)
...
alloc = np.load(cfg.dir_name + "/alloc_tst_" + str(iter) + ".npy").reshape(D,D,2)
...
plt.title('Revenue')
plt.xlabel("Buyer 1's bid")
plt.ylabel("Buyer 2's bid")
# plt.ylabel("Seller's bid")
_ = plt.colorbar(img, fraction=0.046, pad=0.04)
...
img = ax.imshow(alloc[::-1, :, 0]+alloc[::-1, :, 1], extent=ext, vmin = 0.0, vmax=1.0, cmap = 'YlOrRd', aspect=asp)
...
if save_plot:
    fig.set_size_inches(4, 3)
    plt.savefig(os.path.join(cfg.dir_name, "alloc" + str(iter) + ".png"), bbox_inches = 'tight', pad_inches = 0.05)
