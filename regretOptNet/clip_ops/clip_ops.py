...
def clip_op_01(x):
    ...
    clip_op = x
    clip_op.clamp_(min=0.0, max=1.0)
    return clip_op
    
def clip_op_12(x):
    # clip_op = tf.assign(x, tf.clip_by_value(x, 1.0, 2.0))
    clip_op = x
    clip_op.clamp_(min=1.0, max=2.0)
    return clip_op
    
def clip_op_23(x):
    # clip_op = tf.assign(x, tf.clip_by_value(x, 2.0, 3.0))
    clip_op = x
    clip_op[:, :, :, 0].clamp_(min=2.0, max=3.0)
    clip_op[:, :, :, 1].clamp_(min=2.0, max=3.0)
    return clip_op
...
def clip_op_12_15(x):
    ...
    clip_op = x
    clip_op[:, :, 0, :].clamp_(min=1.0, max=2.0)
    clip_op[:, :, 1, :].clamp_(min=1.0, max=5.0)
    return clip_op
...
def clip_op_416_47(x):
    ...
    # clip_op = tf.group([clip_op_1, clip_op_2])
    clip_op = x
    clip_op[:, :, :, 0].clamp_(min=4.0, max=16.0)
    clip_op[:, :, :, 1].clamp_(min=4.0, max=7.0)
    return clip_op
...
def clip_op_04_03(x):
    ...
    # clip_op = tf.group([clip_op_1, clip_op_2])
    clip_op = x
    clip_op[:, :, :, 0].clamp_(min=0.0, max=4.0)
    clip_op[:, :, :, 1].clamp_(min=0.0, max=3.0)
    return clip_op
