...
class Trainer(object):
    ...
    def init_param(self):
        ...
        if self.mode == "train":
            ...
            self.opt_3 = torch.optim.SGD([self.b_w_rgt, self.s_w_rgt, self.b_w_irp, self.s_w_irp, self.a_w_irp], lr=self.update_rate, weight_decay=wd)
    ...
    def differentiable_convex(self, alloc, pay, rev):
    ...
    def forward(self, X, X_ADV, Y, Y_ADV):
        ...
        b_rgt = cp.Variable(b_excess_from_utility.amax(dim=3).amax(1).mean(1))
        s_rgt = cp.Variable(s_excess_from_utility.amax(dim=3).amax(1).mean(1))
        rgt = b_rgt + s_rgt

        alloc = cp.Variable(self.alloc)
        b_x = cp.Parameter(X)
        s_x = cp.Parameter(Y)

        opjective = cp.Maximize(torch.mean((torch.multiply(b_x, alloc.sum(-1))).sum(-1) - (torch.multiply(s_x, alloc.sum(-2))).sum(-1)))
        ...
        if self.mode == 'train':
            ...
            lag_loss = (self.b_w_rgt * b_rgt).sum() + \
                       ...
                       (self.a_w_irp * a_irp).sum()       

            loss_1 = -welfare + rgt_penalty + irp_penalty + lag_loss
