...
class Trainer(object):
    ...
    def compute_entropy(self, alloc):
        ...
        unmatched_buyer_alloc = torch.ones([self.config[self.mode].batch_size, self.config.num_buyers]) - alloc.sum(-1)
        unmatched_seller_alloc = torch.ones([self.config[self.mode].batch_size, self.config.num_sellers]) - alloc.sum(-2)
    ...
    def get_misreports(self, x, adv_var):
        num_misreports = adv_var.shape[0]
        adv = adv_var.unsqueeze(0).tile([self.config.num_agents, 1, 1, 1])
        x_mis = x.tile([self.config.num_agents * num_misreports, 1, 1])
        x_r = x_mis.reshape(self.adv_shape)
        y = x_r * (1 - self.adv_mask) + adv * self.adv_mask
        misreports = (y.reshape([-1, self.config.num_agents, self.config.num_items])).float()
    ...
    def init_param(self):
        ...
        # self.x_shape = [self.config[self.mode].batch_size, self.config.num_agents, self.config.num_items]
        ...
        # self.adv_var_shape = [ self.config[self.mode].num_misreports, self.config[self.mode].batch_size, self.config.num_agents, self.config.num_items]
        ...
        # self.u_shape = [self.config.num_agents, self.config[self.mode].num_misreports, self.config[self.mode].batch_size, self.config.num_agents]
        ...
        # self.b_adv_mask = np.concatenate([self.b_adv_mask, np.zeros(self.s_adv_shape)], axis=0)
        ...
        # self.s_adv_mask = np.concatenate([np.zeros(self.b_adv_shape), self.s_adv_mask], axis=0)
        ...
        # self.u_mask = np.zeros(self.u_shape)
        # self.u_mask[np.arange(self.config.num_agents), :, :, np.arange(self.config.num_agents)] = 1.0
        # self.u_mask = torch.tensor(self.u_mask, requires_grad=False)
        # self.adv_mask = self.adv_mask.to(self.device)
        # self.u_mask = self.u_mask.to(self.device)
        # self.adv_mask = self.adv_mask.to(self.device)
        # self.u_mask = self.u_mask.to(self.device)
        ...
        if self.mode == "train":
            ...
            self.w_rgt = torch.tensor(w_rgt_init_val, requires_grad=True)
            

            self.update_rate = self.config.train.update_rate
            self.learning_rate = self.config.train.learning_rate

            # var_list = list(self.net.alloc.parameters()) + list(self.net.pay.parameters())
            # var_list = list(self.net.alloc.parameters()) + list(self.net.alloc_buyer.parameters()) + list(self.net.alloc_seller.parameters()) + list(self.net.pay.parameters()) + list(self.net.rev.parameters())
            
            # Optimizer
            wd = None if "wd" not in self.config.train else self.config.train.wd
            if wd == None:
                wd = 0.0
            # self.opt_1 = torch.optim.Adam([], lr=self.learning_rate, weight_decay=wd)
            # self.opt_3 = torch.optim.SGD([self.w_rgt], lr=self.update_rate, weight_decay=wd)

            # Metrics
            self.metric_names = ["Revenue", "Welfare", "Buyer_Regret", "Seller_Regret", "Reg_Loss", "Lag_Loss", "Net_Loss", "w_rgt_mean", "update_rate"]
            self.w_rgt.to(self.device)
        elif self.mode == "test":
            '''
            ...
            '''
            self.metric_names = ["Revenue", "Welfare","Buyer_Regret", "Seller_Regret", "Buyer_IRP", "Seller_IRP", "Auctioneer_IRP", "Entropy"]
    ...
    def forward(self, X, X_ADV, Y, Y_ADV):
        ...
        self.alloc, self.pay, self.rev = self.net.inference(X, Y)
        # self.alloc, self.pay, self.rev = self.net.inference(torch.cat((X, Y), dim=0))

        a_mis, p_mis, r_mis = self.net.inference(x_misreports, y_misreports)
        ...
        b_rgt.to(self.device)
        s_rgt.to(self.device)
        ...
        if self.mode == 'train':
            ...
            irp_penalty = (self.update_rate) * (b_irp * b_irp).sum()/2.0 + (self.update_rate) * (s_irp * s_irp).sum()/2.0 + (self.update_rate) * (a_irp * a_irp)/2.0
            lag_loss = (self.w_rgt * b_rgt).sum() + \
                       (self.w_rgt * s_rgt).sum() + \
                       (self.w_rgt * b_irp).sum() + \
                       (self.w_rgt * s_irp).sum() + \
                       (self.w_rgt * a_irp).sum()              

            loss_1 = -revenue + rgt_penalty + irp_penalty + lag_loss
            loss_2 = -b_u_mis.sum() -s_u_mis.sum()
            loss_3 = -lag_loss

            self.metrics = [revenue, welfare, b_rgt_mean, s_rgt_mean, rgt_penalty,
                            lag_loss, loss_1, self.w_rgt.mean(), self.update_rate]
        ...
        if self.mode == 'test':
            ...
            self.metrics = [revenue, welfare, b_rgt_mean, s_rgt_mean, b_irp_mean, s_irp_mean, a_irp_mean, entropy_mean]
